...
namespace Units
{
inline Vector3 ScreenSpaceToNDC( int screenSpaceX, int screenSpaceY, int width, int height )
{
	const float aspectRatio{ static_cast<float>( width ) / height };
	const float ndcX{ ( 2.f * ( ( static_cast<float>( screenSpaceX ) + 0.5f ) / width ) - 1.f ) };
	const float ndcY{ 1.f - 2.f * ( ( static_cast<float>( screenSpaceY ) + 0.5f ) / height ) };

	return Vector3{ ndcX, ndcY, 1.f };
}
} // namespace Units
namespace Utils
{
